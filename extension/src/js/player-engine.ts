...
export class PlayerEngine {
	...
	constructor() {
		...
		this.pixiApp.view.style.cursor = 'grab';
		
		// Wait for container to be ready before appending
		const appendCanvas = () => {
			const container = document.querySelector(Constants.stageSelector);
			console.log("🐱 PlayerEngine: Looking for container", Constants.stageSelector, container);
			if (container) {
				container.appendChild(this.pixiApp.view);
				console.log("🐱 PlayerEngine: Canvas appended to container");
				this.initializeLoader();
			} else {
				console.log("🐱 PlayerEngine: Container not ready, retrying in 100ms");
